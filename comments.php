...
<div id="comments" class="comments-area">
    ...
    <div id="comments-tabber" class="clearfix">
        <a id="reviews-tab" class="curtab" rel="nofollow"><span><?php echo $co_count; ?></span> 條評論</a>
    <?php if ($tb_count != 0): ?>
    ...
    <?php
        ...
        // custom fields
        $fields = array(
            'author' => '<input id="author" name="author" type="text" placeholder="暱稱*" value="' . esc_attr($comment_author) . '" size="30" aria-required="true" /><label for="author">暱稱*</label>',
            'email' => '<input id="email" name="email" type="text" placeholder="郵箱*" value="' . esc_attr($comment_author_email) . '" size="30" aria-required="true" /><label for="author">郵箱*</label>',
            'url' => '<input id="url" name="url" type="text" placeholder="網站" value="' . esc_attr($comment_author_url) . '" size="30" /><label for="author">網站*</label>',
            'email_notify' => '<input type="checkbox" name="comment_mail_notify" id="comment_mail_notify" value="" checked="checked" /><label for="comment_mail_notify">有人回復時郵件通知我</label>',
        ...
        if (!is_user_logged_in() && !empty($comment_author)) {
            ...
            $welcome_login .= '<span><strong>' . $comment_author . '</strong>, 歡迎回來.</span>';
        ...
        <div class="comment-pagenavi clearfix">
            <div class="alignright">
                <?php
                    ...
                    $prev_link = get_previous_comments_link('<i class="icon-arrow-circle-left"></i> 舊評論');
                    $next_link = get_next_comments_link('新評論 <i class="icon-arrow-circle-right"></i>');
